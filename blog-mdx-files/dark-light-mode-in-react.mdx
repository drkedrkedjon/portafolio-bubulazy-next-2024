...
lastEdited: 17-05-2024
stage: First version, could be improved
...
Once we are all good and synchronised, if the user wants to endlessly click the button that switches the theme, let’s give him that pleasure.
...
We know that this can be done much better, but let’s reserve that for future adventures once I progress more with my knowledge.

### Doubts

First, I use two useEffect() hooks. That could be squeezed into one with an if statement inside and another useState() named isFirstRender(true). In that case, the code from the first hook where we sync the state could be placed inside the second hook. I’ll try that soon to see what happens afterwards.

> A week later...
>
> I've done it; placed all in one useEffect() and it looks to me that it works better, here is the code:
...
The second one is to set a cookie in local storage with user theme preferences and to load that cookie on subsequent visits. But, it complicates things. And I wonder, how many users prefer a different theme to their own OS default. In my opinion, not many.

And I do not want to set any sweet cookies because I’m in the EU and that GDPR Cookie Banner thingy that pops up everywhere, I do not want that on my personal site. No tracking, no need for cookie banner.

Lastly, I am unable to notice any flash of unstyled HTML or the wrong theme. That doesn’t mean I’m 100% sure that there is no flash of white nirvana in different circumstances.
